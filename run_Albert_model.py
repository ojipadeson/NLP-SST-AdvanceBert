...
def model_train_validate_test(train_df_input, dev_df_input, test_df_input, target_dir_input,
    ...
    Parameters
    ----------
    train_df_input : pandas dataframe of train set.
    dev_df_input : pandas dataframe of dev set.
    test_df_input : pandas dataframe of test set.
    target_dir_input : the path where you want to save model.
    ...
    if not os.path.exists(target_dir_input):
        os.makedirs(target_dir_input)

    # -------------------- Data loading --------------------------------------#

    print("\t* Loading training data...")
    train_data = DataPrecessForSentence(tokenizer, train_df_input, max_seq_len=max_seq_len)
